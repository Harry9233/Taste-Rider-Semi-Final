...
import { ShoppingCart, Search, AlertTriangle, ChevronLeft, PackageOpen, Weight, Plus, Minus } from 'lucide-react';
...
const ChocolatesPage = ({ products, onAddToCart, cartItems, onUpdateQuantity }) => {
  const navigate = useNavigate();
  const chocolateProducts = products.filter(p => p.category === 'Chocolates');
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 md:mb-10 lg:mb-12">
        <div className="flex items-center mb-4 sm:mb-0">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-amber-800 text-shadow-md">Our Signature Chocolates</h1>
        </div>
      </div>
      ...
      {chocolateProducts.length === 0 ? (
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-semibold text-amber-700 mb-2">No Chocolates Found</h2>
          <p className="text-stone-600 mb-6 md:text-lg" style={{ fontFamily: "'Lato', sans-serif"}}>
            It seems this category is currently empty.
          </p>
